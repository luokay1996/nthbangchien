...
// Cấu hình màu sắc Nhóm (Lưu Live)
const groupSettings = {
...
function App() {
  ...
  // State lưu trữ nhóm của từng Team (T1-T10)
  ...
  useEffect(() => {
    fetchData();
    // Đăng ký Realtime cho cả bảng thành viên và bảng nhóm
    const channel = supabase.channel('db-live-updates')
  ...
  // Hàm cập nhật nhóm lên Supabase (Chỉ dành cho Admin)
  const updateTeamGroup = async (teamId, groupName) => {
    if (!isAdmin) return;
    ...
    if (error) console.error("Lỗi lưu nhóm:", error);
  };
  ...
  return (
    <div style={{ backgroundColor: '#000', color: 'white', minHeight: '100vh', padding: '15px', textAlign: 'center', fontFamily: 'Arial' }}>
      ...
      <div style={{ position: 'absolute', top: '10px', right: '10px', zIndex: 100 }}>
        <button onClick={handleAdminLogin} style={{ background: isAdmin ? 'gold' : 'transparent', color: isAdmin ? '#000' : 'gold', border: '1px solid gold', padding: '5px 10px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold' }}>
      ...
      <form onSubmit={handleSubmit} style={{ marginBottom: '25px' }}>
        <input style={{ padding: '10px', background: '#111', color: 'white', border: '1px solid #333', borderRadius: '4px', width: '150px' }} placeholder="Tên..." value={form.char_name} onChange={e => setForm({...form, char_name: e.target.value})} required />
        ...
        <button type="submit" style={{ padding: '10px 15px', background: 'gold', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>ĐĂNG KÝ {form.team_slot ? `(S${form.team_slot})` : ''}</button>
      </form>

      {/* GRID CHÍNH THỨC */}
      <h2 style={{ color: 'gold', fontSize: '15px', marginBottom: '10px' }}>ĐỘI HÌNH CHÍNH THỨC</h2>
      <div className="team-grid">
        {[...Array(10)].map((_, col) => {
          const teamId = col + 1;
          const currentGroup = teamGroups[teamId] || 'Nhóm 1';
          const settings = groupSettings[currentGroup];
          return (
            <div key={col} style={{ 
              background: settings.bg, padding: '8px', borderRadius: '8px', border: `2px solid ${settings.border}`, transition: 'all 0.3s ease'
            }}>
              <div style={{ marginBottom: '6px' }}>
                <span style={{ color: settings.label, fontSize: '11px', fontWeight: 'bold' }}>TEAM {teamId}</span>
                <select 
                  className="group-select"
                  style={{ borderColor: settings.border }}
                  value={currentGroup}
                  disabled={!isAdmin}
                  onChange={(e) => updateTeamGroup(teamId, e.target.value)}
                >
      ...
      {/* POPUP THÀNH VIÊN */}
      {selectedMember && (
        <div style={{ position: 'fixed', bottom: '20px', left: '50%', transform: 'translateX(-50%)', background: '#1a1a1a', padding: '15px', borderRadius: '10px', border: '2px solid gold', zIndex: 1000, minWidth: '250px' }}>
          <div style={{ color: 'gold', fontWeight: 'bold', marginBottom: '10px' }}>{selectedMember.char_name}</div>
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
            <button onClick={async () => {
              await supabase.from('register_list').update({ has_item: !selectedMember.has_item }).eq('id', selectedMember.id);
              setSelectedMember(null);
            }} style={{ background: '#28a745', color: 'white', border: 'none', padding: '8px 15px', borderRadius: '4px' }}>📦 VẬT TƯ</button>
            <button onClick={async () => {
              if(window.confirm("Xóa thành viên?")) await supabase.from('register_list').delete().eq('id', selectedMember.id);
              setSelectedMember(null);
            }} style={{ background: '#dc3545', color: 'white', border: 'none', padding: '8px 15px', borderRadius: '4px' }}>XÓA</button>
            <button onClick={() => setSelectedMember(null)} style={{ background: '#444', color: 'white', border: 'none', padding: '8px 15px', borderRadius: '4px' }}>ĐÓNG</button>
          </div>
        </div>
      )}
    </div>
