...
// Khởi tạo Supabase Client an toàn với Fallback String
  ...
  const fetchData = useCallback(async () => {
    try {
      const [
        ...
        supabase.from('skill_library').select('*')
      ]);
      ...
      console.error("Lỗi khi tải dữ liệu Supabase:", err);
    }
  }, []);
...
// --- HÀM THÊM MARKER MỚI (ĐÃ SỬA LỖI VÒNG TRÒN ĐOÀN & TỨC THÌ UPDATE) ---
  ...
  const startAssignMember = (member) => {
    ...
    setSelectedClassForAssign(member.class_name || 'Toái Mộng'); 
  };
  ...
  // Chỉ xóa khỏi ô sơ đồ (Đưa về danh sách chờ)
 ...
 return (
    <div className="app-container" style={{ backgroundColor: '#000', color: 'white', minHeight: '100vh', padding: '15px', textAlign: 'center', fontFamily: 'Arial, sans-serif', userSelect: 'none' }}>
      <style>{`
        /* --- GIAO DIỆN MOBILE / MẶC ĐỊNH --- */
        .app-container { max-width: 100%; margin: 0 auto; }
        .header-stats { display: flex; justify-content: center; gap: 5px; background: #0a0a0a; padding: 10px; border-radius: 8px; border: 1px solid #222; margin-bottom: 15px; flex-wrap: wrap; }
        ...
        .marker-remove-btn { position: absolute; top: -6px; right: -6px; background: red; color: white; border: none; border-radius: 50%; width: 16px; height: 16px; font-size: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-weight: bold; z-index: 20; }
        ...
        .skill-box { width: 55px; height: 55px; background: #222; border: 1px solid #444; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 10px; color: #555; position: relative; overflow: visible; }
        .skill-lib-item-container { position: relative; }
        .skill-lib-item { width: 50px; height: 50px; cursor: pointer; border-radius: 6px; border: 1px solid #333; transition: transform 0.1s; object-fit: cover; }
        ...
        .custom-add-skill { display: flex; gap: 5px; margin-top: 10px; padding: 0; }
        .custom-input { flex: 1; background: #000; border: 1px solid #444; color: white; padding: 6px; border-radius: 4px; }
        .upload-btn { background: #333; color: white; border: 1px solid #555; padding: 6px 10px; border-radius: 4px; cursor: pointer; font-weight: bold; display: inline-block; }
        ...
        .waiting-item-chip { display: flex; align-items: center; justify-content: space-between; height: 28px; padding: 0 6px 0 10px; border-radius: 4px; font-size: 11px; font-weight: bold; overflow: hidden; border: 1px solid rgba(255,255,255,0.1); cursor: pointer; }
        .waiting-text-name { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1; margin-right: 4px; }
        .waiting-delete-icon-btn { background: rgba(0, 0, 0, 0.4); color: #ff4d4d; border: none; border-radius: 3px; width: 16px; height: 16px; font-size: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-weight: bold; flex-shrink: 0; transition: all 0.2s; }
        ...
        /* --- PHÓNG TO TRÊN MÀN HÌNH MÁY TÍNH --- */
        @media (min-width: 1400px) {
          .app-container { max-width: 98vw; padding: 25px !important; }
          h1 { font-size: 36px !important; margin: 20px 0 !important; }
          .header-stats { padding: 18px !important; font-size: 16px !important; gap: 15px !important; }
          .team-grid { max-width: 100% !important; gap: 12px !important; }
          .slot-cell { height: 62px !important; font-size: 15px !important; margin: 5px 0 !important; }
          .leader-icon { font-size: 13px !important; top: 3px !important; left: 4px !important; }
          .group-select { font-size: 14px !important; padding: 6px !important; margin-top: 8px !important; }
          .waiting-box { max-width: 100% !important; padding: 25px !important; }
          .waiting-item-chip { height: 38px !important; font-size: 15px !important; }
          .waiting-flex-grid { grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)) !important; gap: 10px !important; }
          .map-section { max-width: 1300px !important; }
        }
      `}</style>

      {/* THANH ĐIỀU KHIỂN BẢO MẬT & RESET ADMIN */}
      <div style={{ position: 'absolute', top: '10px', right: '10px', display: 'flex', flexDirection: 'column', gap: '5px', alignItems: 'flex-end', zIndex: 100 }}>
        <button onClick={handleAdminLogin} style={{ background: isAdmin ? '#d4af37' : 'transparent', color: isAdmin ? '#000' : '#d4af37', border: '1px solid #d4af37', padding: '5px 10px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>
          {isAdmin ? "MODE: ADMIN CONTROL" : "ADMIN LOGIN"}
        </button>
        {isAdmin && (
          <button onClick={handleResetBoard} style={{ background: '#d32f2f', color: 'white', border: 'none', padding: '5px 10px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>
            RESET DATA WEEK
          </button>
        )}
      </div>

      <img src="/nth-logo.png" alt="Logo" style={{ width: '280px', margin: '0 auto', display: 'block' }} />
      <h1 style={{ color: 'gold', fontSize: '22px', margin: '10px 0', letterSpacing: '1px' }}>♅ BANG THIÊN PHẠT ♅</h1>

      {/* THỐNG KÊ QUÂN SỐ VÀ HỆ PHÁI */}
      <div className="header-stats">
        {Object.keys(classInfo || {}).map(cls => (
          <div key={cls} style={{ borderRight: '1px solid #222', paddingRight: '6px', minWidth: '55px' }}>
            <div style={{ color: classInfo[cls]?.color || '#fff', fontSize: '10px', fontWeight: 'bold' }}>{cls}</div>
            <div style={{ fontSize: '14px', fontWeight: 'bold' }}>
              {members.filter(m => m.class_name === cls && m.team_slot).length}
            </div>
          </div>
        ))}
        <div style={{ paddingLeft: '8px', borderLeft: '2px solid #333', display: 'flex', gap: '15px', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: '10px', fontWeight: 'bold', color: '#00FF00' }}>QUÂN SỐ</div>
            <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#00FF00' }}>{officialCount} / 60</div>
          </div>
          <div>
            <div style={{ fontSize: '10px', fontWeight: 'bold', color: '#ff5e00' }}>📦 VẬT TƯ</div>
            <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#ff6600' }}>{totalItemsCount}</div>
          </div>
          <div>
            <div style={{ fontSize: '10px', fontWeight: 'bold', color: '#00a2ff' }}>🔎 SCOUT</div>
            <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#00ffff' }}>{totalScoutsCount}</div>
          </div>
          <div>
            <div style={{ fontSize: '10px', fontWeight: 'bold', color: '#ff4500' }}>🔨 TRỤ</div>
            <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#ff4500' }}>{totalTowersCount}</div>
          </div>
        </div>
      </div>
      ...
      {/* SƠ ĐỒ ĐỘI HÌNH CHÍNH (10 TEAM x 6 THÀNH VIÊN) */}
     ...
     {/* BẢN ĐỒ CHỈ ĐẠO CHIẾN THUẬT */}
      <div className="map-section">
        ...
        {isAdmin && (
          <div className="admin-map-controls">
            ...
            <button className="control-btn" style={{ background: '#d400ff', color: '#fff' }} onClick={() => addNewMarker('Đoàn 2')}>+ Vòng tròn Đoàn 2</button>
      ...
      {/* MODAL GÁN THÀNH VIÊN CHO ADMIN */}
        ...
        <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', background: '#141414', padding: '20px', borderRadius: '12px', border: '2px solid gold', zIndex: 1100, width: '90%', maxWidth: '520px', boxShadow: '0 0 40px rgba(0,0,0,0.9)', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
          ...
          {!pendingAssignMember && (
            <div style={{ borderTop: '1px solid #222', paddingTop: '10px', marginBottom: '12px', textAlign: 'left' }}>
              ...
              <textarea 
                style={{ width: '100%', height: '60px', background: '#000', border: '1px solid #444', borderRadius: '4px', color: '#fff', padding: '5px', fontSize: '11px', resize: 'none', fontFamily: 'monospace' }}
      ...
      {/* MODAL CHI TIẾT PROFILE & CẤU HÌNH SKILL */}
      {selectedMember && (
        <div style={{ position: 'fixed', bottom: '20px', left: '50%', transform: 'translateX(-50%)', background: '#1a1a1a', padding: '20px', borderRadius: '15px', border: '2px solid gold', zIndex: 1000, width: '90%', maxWidth: '420px', boxShadow: '0 0 30px rgba(0,0,0,1)' }}>
          <div style={{ marginBottom: '2px', fontWeight: 'bold', color: classInfo[selectedMember.class_name]?.color || '#fff', fontSize: '18px' }}>{selectedMember.char_name}</div>
          <div style={{ fontSize: '11px', color: '#888', marginBottom: '15px' }}>Hệ: {selectedMember.class_name || 'Chưa chọn'} | {selectedMember.type}</div>

          {/* KHO SKILL KHI LÀ ADMIN */}
          {isAdmin && (
            <div style={{ marginBottom: '15px', background: '#222', padding: '10px', borderRadius: '8px' }}>
              <div style={{ fontSize: '10px', color: 'gold', marginBottom: '8px', fontWeight: 'bold' }}>KHO SKILL CHUNG (ADMIN)</div>
              <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap', maxHeight: '110px', overflowY: 'auto', padding: '5px' }}>
              ...
              {[0, 1, 2, 3, 4].map(i => {
                ...
                return (
                  <div key={i} className="skill-box">
                    {skill ? (
                      <>
                        <img src={skill.skill_url} style={{ width: '100%', height: '100%', borderRadius: '6px', objectFit: 'cover' }} alt="equipped" />
                        {isAdmin && (
          ...
          {isAdmin && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px', marginBottom: '10px' }}>
              <button type="button" onClick={toggleScout} style={{ background: selectedMember.is_scout ? '#00ffff' : '#333', color: selectedMember.is_scout ? '#000' : '#fff', border: 'none', padding: '8px', borderRadius: '6px', fontWeight: 'bold', fontSize: '11px', cursor: 'pointer' }}>
                {selectedMember.is_scout ? "BỎ SCOUT 🔎" : "SCOUT 🔎"}
              </button>
              <button type="button" onClick={toggleTowerTeam} style={{ background: selectedMember.is_tower_team ? '#ff4500' : '#fff', color: selectedMember.is_tower_team ? '#fff' : '#000', border: 'none', padding: '8px', borderRadius: '6px', fontWeight: 'bold', fontSize: '11px', cursor: 'pointer' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            {isAdmin && (
              <>
                <button type="button" onClick={toggleItem} style={{ flex: 1, background: selectedMember.has_item ? '#444' : '#28a745', color: 'white', border: 'none', padding: '10px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}>
                  {selectedMember.has_item ? "BỎ VẬT TƯ" : "VẬT TƯ 📦"}
                </button>
                <button type="button" onClick={deleteMember} style={{ background: '#dc3545', color: 'white', border: 'none', padding: '10px', borderRadius: '6px', fontWeight: 'bold', fontSize: '12px', cursor: 'pointer' }}>XÓA KHỎI Ô</button>
              </>
            )}
            <button type="button" onClick={() => setSelectedMember(null)} style={{ flex: !isAdmin ? 1 : 'none', background: '#333', color: 'white', border: 'none', padding: '10px', borderRadius: '6px', fontWeight: 'bold', minWidth: '80px', cursor: 'pointer' }}>ĐÓNG</button>
          </div>
        </div>
      )}
    </div>
